 ...
 */
public class ChallengesMasterclass {
...
class ArraySorter {
    ...
    // unique object behavior/class methods

    /**
     * builds an array of integers captured from user input
     * @return int[]
     */
    public int[] getIntegers(int capacity) {
        ...
        for(int i = 0; i < intArray.length; i++) {
            ...
            // validate code only runs if user entered an integer
            if(scanner.hasNextInt()) {

                intArray[i] = scanner.nextInt();

                // handle 'enter' key down to continue
                scanner.nextLine();
    ...
    /**
     * sort array and return a new array containing the sorted numbers
     */
    private int[] sortIntegers(int[] intArray) {

        // insertion sort: [[sortedPartition][unsortedPartition]]
        // * sorted partition grows by having unsortedElement search for correct position in the sortedPartition

        // outer loop that builds the sortedPartition
        for(int firstUnsortedIndex = 1; firstUnsortedIndex < intArray.length; firstUnsortedIndex++) {

            int newElement = intArray[firstUnsortedIndex];
            int i;

            // inner loop that handles searching comparison
            ...
            // found correct index for newElement after shifting
    ...
    /**
     * print content of the built array
     */
    public void printArray(int[] intArray) {

        int[] sortedArray = sortIntegers(intArray);

        int[] highestToLowest = new int[sortedArray.length];
        int count = 0;

        for(int i = (sortedArray.length - 1); i >= 0; i--) {
            highestToLowest[count] = sortedArray[i];
            count += 1;
        }

        System.out.println(Arrays.toString(highestToLowest));
    }
}

class MinElement {

    private static final String USER_PROMPT = "Please enter a number:\n";
    ...
    public int findMin(int[] userIntegers) {
        ...
        return selectionSort(userIntegers)[0];
